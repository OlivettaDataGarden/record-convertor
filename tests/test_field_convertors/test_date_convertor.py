...
def test_convert_yyyy_mm_dd():
    ...
    assert convertor.format_date_field(
        **BASE_PARAMS_YYYY_MM_DD)["date"] == "2021-02-21"
...
def test_nested_convert_dd_mm_yyyy():
    ...
    convertor = DateFieldConvertor()
    assert convertor.format_date_field(
        **BASE_PARAMS_NESTED_DATE_FIELD)["date"]["nested_date"] == "2021-02-21"
...
def test_convert_yyyy_mm_dd_underscore():
    ...
    convertor = DateFieldConvertor()
    assert convertor.format_date_field(
        **BASE_PARAMS_YYYY_MM_DD_UNDERSCORE)["date"] == "2021-02-21"
...
def test_convert_yyyy_mm_dd_time():
    ...
    convertor = DateFieldConvertor()
    assert convertor.format_date_field(
        **BASE_PARAMS_YYYY_MM_DD_Time)["date"] == "2021-02-21"
...
def test_convert_unix_date_time_stamp():
    ...
    convertor = DateFieldConvertor()
    assert convertor.format_date_field(
        **BASE_PARAMS_UNIX_DT_STAMP)["date"] == "2021-02-21"
...
def test_convert_no_date_field():
    ...
    convertor = DateFieldConvertor()
    assert convertor.format_date_field(
        **BASE_PARAMS_NONE_DATE)["date"] is None
