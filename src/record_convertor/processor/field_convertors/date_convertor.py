...
import jmespath
from ...package_settings import EvaluateConditions
...
class DateFieldConvertor:
    ...
    def convert_date(self) -> Dict:
        ...
        """
        date_formatter = CONV_METHODS.get(self.conversion_rule.get("format", ""), "")
        if not date_formatter:
            raise NotImplementedError(f"Format date convertor {date_formatter}")
        date_field_value = self._get_field()

        if date_field_value and self.all_conditions_true(date_field_value):
            date_in_new_format = getattr(self, date_formatter)(date_field_value)
