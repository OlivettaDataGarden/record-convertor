"""
This module provides utility functions for manipulating dictionaries and lists, particularly focusing on case normalization and cleaning of data structures. It includes functions for recursively converting dictionary keys to lowercase, even within nested dictionaries and lists, and for removing entries from dictionaries where values are None.

Functions:
    - list_in_lower_case: Recursively converts all dictionary keys within a list to lower case, including keys in nested structures.
    - keys_in_lower_case: Recursively converts all keys in a dictionary to lower case, including keys in nested dictionaries and lists.
    - dict_without_non_values: Removes key-value pairs from a dictionary where the value is None, cleaning up the dictionary.

The module aims to facilitate the handling of data structures that are commonly used in data parsing, preprocessing, and transformation tasks, ensuring consistency in key naming conventions and eliminating null entries from dictionaries.
"""
